...
aiSummary: >
  ...
  可选 defaultTabs 声明常驻默认 Tab，决定初始 tabs、初始 selectedTab 与 resetCustomTab 落点；不传时 tabs 为空、selectedTab 为 null。
...
function useCustomTabProvider<T extends Record<string, unknown>>(options: {
  ...
  // 常驻默认 Tab（如 ChatContainer 的「文件产物」），决定初始列表、初始选中与 reset 落点，缺省为空
...
const { tabs, selectedTab, isCollapse, addCustomTab, ensureCustomTab, removeCustomTab, selectCustomTab, resetCustomTab } =
  useCustomTabProvider({
  // 常驻默认 Tab：初始即在 Tab 栏并默认选中，resetCustomTab 也回退到这里
...
// 仅确保 Tab 存在（不展开、不切换选中）—— 如侧栏已打开时同步挂上新 Tab
...
| selectedTab     | `Ref<CustomTab \| null>`    | 当前选中的 Tab，初始为 `defaultTabs[0]`（缺省 `null`）；未被主动切换过时跟随 Tab 栏首位，选中项被移除或隐藏时自动回退到首个可见 Tab |
...
| resetCustomTab  | `() => void`                  | 重置为仅保留 `defaultTabs`、折叠侧栏并选中 `defaultTabs[0]`（缺省 `null`）；`ChatContainer` 在卸载时调用，避免残留自定义 Tab |
...
interface CustomTab<T = Record<string, unknown>> {
  ...
  visible?: boolean;
  /** 是否可关闭，缺省 true；常驻默认 Tab 一般显式传 false */
  closable?: boolean;
}
```
...
- 无 Tab 被特殊对待：显隐统一按 `tab.visible !== false` 判断；常驻能力由 `defaultTabs` 表达，`ChatContainer` 传入「文件产物」Tab
- `displayTabs` 负责「过滤显隐 + 按 `order` 排序」；原始 `tabs` 仍保留全部 Tab 供程序化选中与查找
- 排序为稳定排序，`order` 相同的 Tab 保持插入先后顺序
- 选中的 Tab 被移除或被配置隐藏时，内容不再渲染，自动切换到 `displayTabs` 首位
- `addCustomTab` 同时展开侧边栏（`isCollapse = false`）并在 `nextTick` 后自动选中目标 Tab；同名 Tab 合并更新
- `ensureCustomTab` 与 `addCustomTab` 共用合并逻辑，但不改 `isCollapse`、自身不切换 `selectedTab`；适合「侧栏已因其他 Tab 打开时同步挂上新 Tab」等场景
...
- [useArtifactPreview](./use-artifact-preview) — 文件产物 Tab：由 `ChatContainer` 的 `defaultTabs` 常驻声明，点击卡片时 `addCustomTab` 展开并选中
